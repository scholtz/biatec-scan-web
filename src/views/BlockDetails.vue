    ...
    <div v-else-if="block">
      ...
      <div class="card mb-8">
        <div class="flex items-center justify-between mb-6">
          <h1 class="text-3xl font-bold text-white">Block #{{ block.round.toLocaleString() }}</h1>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <div>
            ...
            <p class="text-white font-medium">{{ new Date(block.timestamp * 1000).toLocaleString() }}</p>
          </div>
          <div>
          ...
          <div>
            ...
            <p class="text-white font-medium">{{ block['txn-counter'].toLocaleString() }}</p>
          </div>
          <div>
            <p class="text-sm text-gray-400 mb-1">Genesis ID</p>
            <p class="text-white font-medium font-mono text-sm">{{ block['genesis-id'] }}</p>
          </div>
        </div>
        ...
        <div class="mt-6 pt-6 border-t border-dark-700">
          <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div>
              ...
              <p class="text-white font-mono text-sm bg-dark-900 p-3 rounded border break-all">
                {{ block['previous-block-hash'] || 'Genesis Block' }}
              </p>
            </div>
            <div>
      ...
      <div class="flex justify-between items-center mb-8">
        <router-link 
          v-if="block.round > 1"
          :to="{ name: 'BlockDetails', params: { round: block.round - 1 } }"
      ...
      <div>
        ...
        <div v-else-if="transactions.length" class="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
...
const block = ref<AlgorandBlock | null>(null);
...
const loadBlock = async (round: number) => {
...
watch(() => route.params.round, (newRound) => {
  if (newRound) {
    loadBlock(Number(newRound));
  }
});
