  ...
  <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div v-else-if="transaction">
      ...
      <div class="card mb-8">
        <div class="flex items-center justify-between mb-6">
          <div class="flex items-center space-x-4">
            <div class="w-12 h-12 rounded-full flex items-center justify-center" :class="getTypeColor(transaction['tx-type'])">
              <span class="font-bold text-lg">{{ getTypeIcon(transaction['tx-type']) }}</span>
            </div>
            <div>
              <h1 class="text-2xl font-bold text-white">{{ getTypeLabel(transaction['tx-type']) }}</h1>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <div>
            ...
            <router-link 
              :to="{ name: 'BlockDetails', params: { round: transaction['confirmed-round'] } }"
          ...
          <div>
            ...
            <p class="text-white font-medium">{{ algorandService.formatAlgoAmount(transaction.fee) }} ALGO</p>
          </div>
          <div>
            <p class="text-sm text-gray-400 mb-1">Round Time</p>
            <p class="text-white font-medium">
              {{ transaction['round-time'] ? new Date(transaction['round-time'] * 1000).toLocaleString() : 'Unknown' }}
            </p>
          </div>
          <div>
            <p class="text-sm text-gray-400 mb-1">Valid Rounds</p>
            <p class="text-white font-medium">{{ transaction['first-valid'] }} - {{ transaction['last-valid'] }}</p>
          </div>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
          ...
          <div class="space-y-4">
            <div>
              ...
              <div class="bg-dark-900 p-3 rounded border">
                <p class="text-white font-mono text-sm break-all">{{ transaction.sender }}</p>
              </div>
            </div>
            <div v-if="getReceiver(transaction)">
              <p class="text-sm text-gray-400 mb-2">To</p>
              <div class="bg-dark-900 p-3 rounded border">
                <p class="text-white font-mono text-sm break-all">{{ getReceiver(transaction) }}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <!-- Amount Info -->
        <div class="card">
          <h2 class="text-lg font-semibold text-white mb-4">Transaction Amount</h2>
          ...
          <div v-else-if="transaction['asset-transfer-transaction']" class="space-y-2">
            <p class="text-2xl font-bold text-blue-400">
              {{ transaction['asset-transfer-transaction'].amount.toLocaleString() }}
            </p>
            <p class="text-sm text-gray-400">Asset ID: {{ transaction['asset-transfer-transaction']['asset-id'] }}</p>
          </div>
      ...
      <div class="card">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <div class="bg-dark-900 p-3 rounded border">
              <p class="text-white font-mono text-xs break-all">{{ transaction['genesis-hash'] }}</p>
            </div>
          </div>
          <div>
            <p class="text-sm text-gray-400 mb-2">Genesis ID</p>
            <div class="bg-dark-900 p-3 rounded border">
              <p class="text-white font-mono text-sm">{{ transaction['genesis-id'] }}</p>
            </div>
          </div>
          <div>
            <p class="text-sm text-gray-400 mb-2">Intra Round Offset</p>
            <p class="text-white font-medium">{{ transaction['intra-round-offset'] }}</p>
          </div>
          <div>
      ...
      <div v-if="transaction['application-transaction']" class="card mt-8">
        <h2 class="text-lg font-semibold text-white mb-4">Application Call Details</h2>
        <div class="space-y-4">
          <div>
            <p class="text-sm text-gray-400 mb-1">Application ID</p>
            <p class="text-white font-medium">{{ transaction['application-transaction']['application-id'] }}</p>
          </div>
          <div>
            <p class="text-sm text-gray-400 mb-1">On Completion</p>
            <p class="text-white font-medium">{{ transaction['application-transaction']['on-completion'] }}</p>
          </div>
          <div v-if="transaction['application-transaction']['application-args']?.length">
    ...
    <div v-else class="card text-center py-12">
      <h2 class="text-xl font-semibold text-white mb-2">Transaction Not Found</h2>
