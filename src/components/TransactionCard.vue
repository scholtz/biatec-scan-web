  ...
  <div class="card">
    <div class="flex items-center justify-between mb-4">
      <div class="flex items-center space-x-3">
        <div class="w-10 h-10 rounded-full flex items-center justify-center" :class="getTypeColor(transaction['tx-type'])">
          <span class="font-semibold text-sm">{{ getTypeIcon(transaction['tx-type']) }}</span>
        </div>
        <div>
          <h3 class="font-medium text-white">{{ getTypeLabel(transaction['tx-type']) }}</h3>
          <p class="text-xs text-gray-400">Block #{{ transaction['confirmed-round']?.toLocaleString() }}</p>
        </div>
      </div>
