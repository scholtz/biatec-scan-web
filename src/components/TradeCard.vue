<template>
  <div class="card border-l-4" :class="trade.type === 'buy' ? 'border-l-green-500' : 'border-l-red-500'">
    <div class="flex items-center justify-between mb-3">
      <div class="flex items-center space-x-3">
        <div class="w-8 h-8 rounded-full flex items-center justify-center" 
             :class="trade.type === 'buy' ? 'bg-green-600' : 'bg-red-600'">
          <span class="text-white text-xs font-bold">{{ trade.type === 'buy' ? '↗' : '↘' }}</span>
        </div>
        <div>
    ...
    <div class="mb-3">
      ...
      <p class="text-white text-sm font-mono bg-dark-900 p-2 rounded border truncate">
        {{ trade.pool }}
      </p>
    </div>
