  ...
  <div class="relative">
    ...
    >
      <button
        ...
        class="w-full px-3 py-2 text-left text-sm text-white hover:bg-dark-700/60 transition-colors duration-200 first:rounded-t-lg last:rounded-b-lg flex items-center space-x-2"
...
const currentLocaleData = computed(() => availableLocales.find(l => l.code === locale.value))
...
const detectBrowserLanguage = (): string => {
  ...
  if (availableLocales.some(l => l.code === browserLanguage)) {
...
onMounted(() => {
  // Set initial locale based on browser language or saved preference
